...
log = pd.DataFrame(
    columns=['Date', 'Features', 'Model Name', 'Parameter Grid', 'Opt. Hyperparameters', 'Training Percent',
             'Featurization Time (sec)', 'Training Time (sec)', 'Tuning Time (sec)', 'R^2', 'MSE', 'RMSE'])
...
class param:
    ...
    @staticmethod
    def Adaparamgrid():
        base_estimator = [tree.DecisionTreeRegressor(max_features='sqrt', splitter='best', max_depth=3),
                          GradientBoostingRegressor(), SVR(kernel='linear'), RandomForestRegressor(n_estimators=500)]
...
def runRegress(model, model_name, train_features, test_features, train_target, test_target, expt):
    ...
    pva, time_var = predict(model, model_name, train_features, test_features, train_target, test_target, expt)
...
def RegressHyper(train_features, test_features, train_target, test_target, csv_file, target_colname, model_name, folds,
                 iters, expt):
    if model_name == 'rf':
        model = RandomForestRegressor()
        params, param_dict, time_par = hyperTune(model, model_name, train_features, train_target, param.RFparamgrid(),
                                                 folds, iters, expt)
        model = RandomForestRegressor(n_estimators=params['n_estimators'], max_features=params['max_features'],
                                      ...
                                      random_state=25)
        r2_dict, mse_dict, rmse_dict, time_var = runRegress(model, model_name, train_features, test_features,
                                                            train_target, test_target, expt)
    elif model_name == 'svr':
        model = SVR()
        params, param_dict, time_par = hyperTune(model, model_name, train_features, train_target, param.SVRparamgrid(),
                                                 folds, iters, expt)
        ...
        model = GradientBoostingRegressor()
        params, param_dict, time_par = hyperTune(model, model_name, train_features, train_target, param.GDBparamgrid(),
                                                 folds, iters, expt)
        ...
        model = AdaBoostRegressor()
        params, param_dict, time_par = hyperTune(model, model_name, train_features, train_target, param.Adaparamgrid(),
                                                 folds, iters, expt)
        model = AdaBoostRegressor(base_estimator=params['base_estimator'], n_estimators=params['n_estimators'],
                                  learning_rate=params['learning_rate'], random_state=25)
        r2_dict, mse_dict, rmse_dict, time_var = runRegress(model, model_name, train_features, test_features,
                                                            train_target, test_target, expt)
    elif model_name == 'nn':
        model = MLPRegressor()
        params, param_dict, time_par = hyperTune(model, model_name, train_features, train_target, param.MLPparamgrid(),
                                                 folds, iters, expt)
        model = MLPRegressor(hidden_layer_sizes=params['hidden_layer_sizes'], activation=params['activation'],
        ...
        model = KNeighborsRegressor()
        params, param_dict, time_par = hyperTune(model, model_name, train_features, train_target, param.KNNparamgrid(),
                                                 folds, iters, expt)
...
class RunML:
    ...
    def Default(csv_file, target_colname, model_name, selected_feat=None):
        feat_sets = feature_select(csv_file, model_name, selected_feat)
        smiles_col = csvhandling.findsmiles(csv_file)
        ...
        for i in index:
            ...
            for comb in combina:
                ...
                for i in feature_lst:
                    ...
                    train_features, test_features, train_target, test_target = train_test_split(featuresarr,
                                                                                                df[target_colname],
                                                                                                test_size=0.2,
                                                                                                random_state=42)
                r2_dict, mse_dict, rmse_dict, train_time = RegressNorm(train_features, test_features, train_target,
                                                                       test_target, csv_file, target_colname,
                                                                       model_name, expt)
            ...
            for comb in combina:
                ...
                for i in feature_lst:
                    ...
                    train_features, test_features, train_target, test_target = train_test_split(featuresarr,
                                                                                                ...
                                                                                                test_size=0.2,
                                                                                                random_state=42)
                r2_dict, mse_dict, rmse_dict, param_dict, train_time, hyper_time = RegressHyper(train_features,
                                                                                                test_features,
                                                                                                train_target,
                                                                                                test_target, csv_file,
                                                                                                target_colname,
                                                                                                model_name, folds,
                                                                                                iters, expt)
...
RunML.Default('dataFiles/Lipophilicity-ID.csv', 'exp', 'nn')
