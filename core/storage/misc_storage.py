...
def store(model):
    ...
    for k, v in tqdm(d.items(), desc="Export to JSON", position=0):
        ...
        if isinstance(v, pd.core.frame.DataFrame) or isinstance(v, pd.core.series.Series):
            if k == "data":  # Rename target column to a more general one
                if model.dataset not in model.multi_label_classification_datasets:
                    v = v.rename(columns={model.target_name: "target"})
                v.to_csv(model.run_name + '_' + k + '.csv')
    ...
    json_name = model.run_name + '_attributes' + '.json'
...
def pickle_model(model):
    ...
    with open(model.run_name + '.pkl', 'wb') as f:
        pickle.dump(model, f)
...
def org_files(model, zip_only=False):
    """
    ...
    try:
        os.mkdir(model.run_name)
    except OSError as e:
        pass

    # put output files into new folder
    if system == "Windows":
        filesp = ''.join(['move ./', model.run_name, '*.* ', model.run_name, '/'])  # move for Windows system
    else:
        filesp = ''.join(['mv ./', model.run_name, '*.* ', model.run_name, '/'])  # mv for Linux system
    ...
    if zip_only:  # remove unzipped folder to avoid duplicates
        if system == "Windows":
            rmdir = 'rmdir ' + model.run_name + ' /s/q'
            ...
            rmdir = os.getcwd() + '/' + model.run_name
...
def decompress_fingerprint(df):
    ...
    fingerprints = fingerprints.str.split(', ', expand=True)  # Split list string into actual list
