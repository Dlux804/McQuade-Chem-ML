...
from exp_core.features import Feature
from numpy.random import randint


class Split(Feature):
    def __init__(self, dataset, target, test=0.2, val=0, random=None):
        Feature.__init__(self, dataset, target)
        if random is None:
    ...
    def data_split(self):
        ...
        print()
        print(
            f"""Dataset of {self.n_tot} points is split into training ({ptrain}%), 
                                                                       validation ({pval}%), and testing ({ptest}%).""")
