...
def graph_gdbparam(model_csv, algor="gdb"):
    ...
    for i in range(len(param_dct)):
        ...
        bf = Relationship(regressor, "learning_rate", learning_rate)
        tx.merge(bf)
        bg = Relationship(regressor, "max_depth", max_depth)
        tx.merge(bg)
        bh = Relationship(regressor, "max_features", max_features)
        tx.merge(bh)
        bi = Relationship(regressor, "min_samples_leaf", min_samples_split)
        tx.merge(bi)
        bj = Relationship(regressor, "min_samples_split", min_samples_split)
        tx.merge(bj)
        bk = Relationship(regressor, "n_estimators", n_estimators)
        tx.merge(bk)
        tx.commit()
    return algo_modeldf
