...
import pathlib
from new_core.models import MLModel
...
def represent_algorithm():
    ...
    :return:
    """
    # Our current supported algorithm
    algorithm_list = ['ada', 'rf', 'svr', 'gdb', 'mlp', 'knn', 'nn', 'svc', 'knc', 'rfc']
...
class Name(MLModel):
    def __init__(self, model):
        super().__init__(algorithm=model.algorithm, dataset=model.dataset, target=model.target, feat_meth=model.feat_meth, cv_folds=model.cv,
                         opt_iter=model.opt_iter, tune=model.tuned)
        self.date = datetime.now().strftime("_%Y%m%d-%H%M%S")
    ...
    def name(self):
        ...
        if self.dataset in dataset_dict.keys():  # If input dataset is one that we have
            ...
            count = 2
            # If this dataset has the same first character as existing ones
            if self.dataset[:-4][0] in dataset_dict.values():
        ...
        print("Created {0} on {1}".format(run_name, self.date))
        return run_name
